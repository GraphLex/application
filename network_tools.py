...
class Source(Enum):
    ...
    NUL = 2 # Empty
    AOT = 3 # Aramaic Old Testament
    LXX = 4 # Septuagint

# irrelevant for future iterations, useful now
...
class NetBuilder:
    def __init__(self):
        ...
        self.hb: pd.DataFrame = pd.read_parquet("resources/hbb.parquet")
    ...
    def translit_to_raw(self, source: Source, lex: str) -> str:
        raw_lemma = ""
        try:
            raw_lemma = self.datasets[source].loc[self.datasets[source]['lemma'] == lex]['display_lemma'].iloc[0]
        except IndexError:
    ...
    def process_strongs_input(self, code: Literal['H', 'G'], num: int) -> str:
        ...
        # TODO: There needs to be a way to map codes to datasets to avoid long if chains.
        if code == 'H':
            return self.datasets[Source.HBB].loc[self.datasets[Source.HBB]['strongno']==f"H{num}"]['lemma'].iloc[0]
        elif code == 'G':
            return self.datasets[Source.GNT].loc[self.datasets[Source.GNT]['strongno']==f"G{num}"]['lemma'].iloc[0]
        else:
            raise ValueError("Incorrect Strong Number")
        
    def _retrain_w2v_model(self, source: Source):
        df = self.hb if source == Source.HBB else self.gnt
